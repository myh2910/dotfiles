...
if [ "$1" = u ]; then
	 notify-send -i microphone-sensitivity-high-symbolic \
		"Mic volume up" \
		"Current mic volume $(ponymix -t source increase 1)%" -t $time
fi
if [ "$1" = d ]; then
	 notify-send -i microphone-sensitivity-low-symbolic \
		"Mic volume down" \
		"Current mic volume $(ponymix -t source decrease 1)%" -t $time
fi
if [ "$1" = i ]; then
...
if [ "$1" = k ]; then
	 notify-send -i audio-volume-high \
		"Global volume up" \
		"Current volume $(ponymix increase 1)%" -t $time
fi

if [ "$1" = j ]; then
	 notify-send -i audio-volume-medium \
		"Global volume down" \
		"Current volume $(ponymix decrease 1)%" -t $time
fi
